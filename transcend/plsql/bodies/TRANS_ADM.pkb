...
IS
   ...
   BEGIN
	 ...
	 set_error_conf( p_name=>'ext_file_missing',
			 p_code=>'The physical file specified by the external table LOCATION does not exist');
	 set_error_conf( p_name=>'on_clause_missing',
			 p_code=>'The ON clause of the MERGE statement was invalid'||chr(10)||'If P_COLUMNS is not provided, then check to see that a primary or unique key exists');
