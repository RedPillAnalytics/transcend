CREATE OR REPLACE PACKAGE BODY common.etl
AS
...
-- operates in an autonomous transaction
   ...
   BEGIN
      ...
      THEN
         ...
         job.log_msg( 'Executing user: ' || l_username );
      ELSE
         job.log_msg( 'Building indexes on ' || p_target_table );
      ...
      FOR c_indexes IN ( SELECT DBMS_METADATA.get_ddl( 'INDEX',
                                                       index_name,
                                                       owner ) index_ddl
      ...
      LOOP
         ...
         l_app.set_action( 'Format index DDL' );
         l_ddl :=
            REGEXP_REPLACE( REGEXP_REPLACE( REGEXP_REPLACE( c_indexes.index_ddl,
                                                            '"',
                                                            NULL ),
                                            '(local.+)?(;)',
                                            NULL,
         ...
         l_app.set_action( 'Execute index DDL' );

         IF p_debug
         THEN
            job.log_msg( 'Index DDL: ' || l_ddl );
         ELSE
      ...
      l_app.set_action( 'Open cursor for all constraints' );

      IF NOT p_debug
      THEN
         job.log_msg( 'Building constraints on ' || p_target_table );
      END IF;

      FOR c_constraints IN ( SELECT DBMS_METADATA.get_ddl( 'CONSTRAINT',
                                                           constraint_name,
      ...
      LOOP
         ...
         -- replace the source owner with the target owner
         -- replace the source table with the target table
         l_app.set_action( 'Format constraint DDL' );
         l_ddl :=
            REGEXP_REPLACE( REGEXP_REPLACE( REGEXP_REPLACE( c_constraints.constraint_ddl,
                                                            '"',
                                                            NULL ),
                                            '(local.+)?(;)',
                                            NULL,
         ...
         THEN
            job.log_msg( 'Index DDL: ' || l_ddl );
         ELSE
            l_app.set_action( 'Execute constraint DDL' );
   ...
   BEGIN
      ...
      l_app.set_action( 'Open cursor for indexes' );
      job.log_msg( 'Dropping indexes on ' || p_table );
   ...
   BEGIN
      ...
      l_app.set_action( 'Construnct insert statement' );
      job.log_msg(    'Loading records from '
                   || p_source_owner
                   || '.'
                   || p_source_object
                   || ' into '
      ...
      IF NOT p_direct
      THEN
         l_sqlstmt := REGEXP_REPLACE( l_sqlstmt,
                                      '/\*\+ APPEND \*/ ',
                                      NULL );
      END IF;

      -- debug statement
      --job.log_msg ('Insert statement: ' || l_sqlstmt);
      EXECUTE IMMEDIATE l_sqlstmt;

      job.log_msg( SQL%ROWCOUNT || ' records loaded' || CHR( 10 ));
   ...
   BEGIN
      ...
      l_app.set_action( 'Generate UPDATE clause' );
      ...
      SELECT REGEXP_REPLACE( stragg( 'target.' || column_name || ' = source.' || column_name ),
        ...
        FROM ( SELECT column_name
               ...
               WHERE table_name = UPPER( p_target_table )
                 AND owner = UPPER( p_target_owner )
              MINUS
              SELECT column_name
                FROM ( SELECT  column_name,
                               MIN( dc.constraint_type ) con_type
      ...
      l_app.set_action( 'Generate INSERT clause' );
      ...
      l_app.set_action( 'Generate VALUES clause' );
      -- construct the "VALUES" clause for the MERGE statement
      l_values := REGEXP_REPLACE( l_insert,
                                  'target.',
                                  'source.' );
      ...
      l_sqlstmt :=
         ...
         || CHR( 10 )
         || l_values;
      job.log_msg(    'Merging records from '
                   || p_source_owner
                   || '.'
                   || p_source_table
                   || ' into '
      ...
      BEGIN
         l_app.set_action( 'Issue MERGE statement' );
      ...
      job.log_msg( SQL%ROWCOUNT || ' records merged' || CHR( 10 ));
      l_app.clear_app_info;
   END merge_tab;
   ...
   BEGIN
      ...
      job.log_msg(    'Loading tables matching regular expression for the '
                   || p_source_owner
                   || ' schema.'
                   || CHR( 10 ));
      ...
      IF p_part_tabs
      THEN
         l_app.set_action( 'Load all objects' );
         ...
         LOOP
            ...
            IF p_debug
            THEN
               job.log_msg(    c_objects.src_owner
                            || '.'
                            || c_objects.src
                            || ' will be loaded into '
         ...
         END LOOP;
      ELSE
         l_app.set_action( 'Load non-partitioned tables and other objects' );
         ...
         LOOP
            ...
            IF p_debug
            THEN
               job.log_msg( 'Running in DEBUG Mode' );
               job.log_msg(    c_objects.src_owner
                            || '.'
                            || c_objects.src
                            || ' will be loaded into '
   ...
   EXCEPTION
      ...
      THEN
         ...
         WHEN 'KUP-04040'
            THEN
               job.log_msg( 'The file does not exist in the directory' );
         ...
         ELSE
               job.log_msg( 'Unknown data cartridge error' );
   ...
   BEGIN
      ...
      ELSE
         BEGIN
            l_app.set_action( 'Determine partition name' );
      ...
      ELSE
         ...
         BEGIN
            ...
            WHEN REGEXP_LIKE( p_stats,
                              ...
                              'i' )
               THEN
                  l_app.set_action( 'Gather stats on new partition' );
         ...
         EXCEPTION
            ...
            THEN
               ...
               job.log_msg(    'No stats existed on '
                            || p_target_owner
                            || '.'
                            || p_target_table
                            || '... gathered new stats' );
      ...
      clone_indexes( p_target_owner,
                     p_target_table,
                     p_source_owner,
                     p_source_table,
                     p_debug );
      l_app.set_action( 'Exchange table' );
      l_sql :=
            'alter table '
         || p_target_owner
         || '.'
      ...
      BEGIN
         ...
         ELSE
            ...
            job.log_msg( p_source_table || ' exchanged for ' || p_target_table );
         END IF;
      ...
      EXCEPTION
         ...
         THEN
            job.log_err;
            job.log_msg( 'Dropping indexes because of exception' );
            drop_indexes( p_source_owner,
                          p_source_table,
                          p_debug );
   ...
   BEGIN
      IF NOT p_debug
      THEN
         job.log_msg(    'Exchanging tables matching the regular expression for the '
                      || p_source_owner
                      || ' schema'
                      || CHR( 10 ));
      END IF;
      ...
      l_app.set_action( 'Open cursor of tables' );
                          ...
                          FROM all_tables tt
                               ...
                               ( SELECT  table_owner,
                                ...
                                GROUP BY table_owner,
                                         table_name ) tp
                               ON( REGEXP_REPLACE( tt.table_name,
                                                   '_[[:alnum:]]+$',
                                                   NULL ) =
                                                 REGEXP_REPLACE( tp.table_name,
                                                                 p_suf_re_rep,
                                                                 NULL ))
                         WHERE REGEXP_LIKE( tt.table_name,
                                            p_regexp,
                                            'i' )
                           AND REGEXP_LIKE( tp.table_name,
                                            p_suf_re_rep,
                                            'i' )
      ...
      LOOP
         ...
         l_rows := TRUE;
         l_app.set_action( 'Exchange tables' );
      ...
      l_app.set_action( 'Check exceptions' );
   ...
   BEGIN
      ...
      l_app.set_action( 'Evaluate global indexes to work on' );

      CASE
         -- error if P_PART_NAME is specified but the table isn't partitioned
      WHEN     l_partitioned = 'NO'
           AND p_part_name IS NOT NULL
         THEN
            raise_application_error( -20001,
                                        p_owner
                                     || '.'
         ...
         THEN
            ...
            FOR c_gidx IN ( SELECT 'alter index ' || owner || '.' || index_name || ' unusable' DDL
                            ...
                            WHERE table_name = UPPER( p_table )
                              ...
                              AND NOT REGEXP_LIKE( index_type,
                                                   'iot',
                                                   'i' )
            ...
            LOOP
               IF p_debug
               THEN
                  job.log_msg( 'Index DDL: ' || c_gidx.DDL );
               ELSE
            ...
            IF l_cnt > 0
            THEN
               job.log_msg( l_cnt || ' global index(es) affected' );
            END IF;
         ELSE
            NULL;
      END CASE;
      ...
      l_cnt := 0;
      l_app.set_action( 'Evaluate indexes to work on' );
      ...
      FOR c_idx IN ( SELECT DISTINCT DDL
                               FROM ( SELECT    'alter index '
                                      ...
                                      WHERE table_name = UPPER( p_table )
                                        AND table_owner = UPPER( p_owner )
                                        AND (    ai.status = 'VALID'
                                              OR aip.status = 'USABLE' )) INDEXES
                              WHERE REGEXP_LIKE( index_type,
                                                 '^' || p_index_type,
                                                 'i' )
                                AND NOT REGEXP_LIKE( index_type,
                                                     'iot',
                                                     'i' )
                                AND REGEXP_LIKE( NVL( part_name, '^' ),
                                                 '^' || p_part_name,
                                                 'i' ))
      LOOP
         IF p_debug
         THEN
            job.log_msg( 'Index DDL: ' || c_idx.DDL );
         ELSE
   ...
   IS
      ...
      l_src_owner   all_tables.owner%TYPE
                                      := NVL( p_src_owner, REGEXP_REPLACE( p_owner,
                                                                           'DW$',
                                                                           'STG' ));
   ...
   BEGIN
      ...
      THEN
         SELECT column_name
          ...
          WHERE NAME = UPPER( p_table )
            AND owner = UPPER( p_owner );
      ELSE
         l_src_col := p_src_col;
      END IF;

      job.log_msg(    'Evaluating '
                   || l_src_owner
                   || '.'
                   || l_src_obj
      ...
      IF p_debug
      THEN
         job.log_msg( 'Dynamic Cursor: ' || l_dyn_ddl );
      END IF;
   ...
   BEGIN
      IF NOT p_debug
      THEN
         job.log_msg( 'Making unusable indexes on ' || p_owner || '.' || p_table || ' usable' );
      END IF;
      ...
      LOOP
         ...
         IF p_debug
         THEN
            job.log_msg( 'Partition DDL: ' || c_idx.DDL );
         ELSE
            EXECUTE IMMEDIATE c_idx.DDL;
         END IF;
      ...
      IF l_cnt > 0
      THEN
         job.log_msg( 'Any unusable local indexes on ' || l_cnt || ' table partitions rebuilt' );
      END IF;
      ...
      LOOP
         ...
         IF p_debug
         THEN
            job.log_msg( 'Partition DDL: ' || c_gidx.DDL );
         ELSE
            EXECUTE IMMEDIATE c_gidx.DDL;
         END IF;
      ...
      IF l_cnt > 0
      THEN
         job.log_msg( l_cnt || ' non-local index(es) affected' );
      END IF;
