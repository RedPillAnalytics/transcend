...
AS
   ...
   BEGIN
      td_dbapi.truncate_table( p_owner      => p_owner, p_table => p_table,
                               p_reuse      => p_reuse );
      o_td.clear_app_info;
   EXCEPTION
      WHEN OTHERS
   ...
   IS
      o_td   tdtype := tdtype( p_module => 'truncate_table' );
   BEGIN
      td_dbapi.drop_table( p_owner => p_owner, p_table => p_table, p_purge => p_purge );
      o_td.clear_app_info;
   EXCEPTION
      WHEN OTHERS
      THEN
   ...
   BEGIN
      td_dbapi.build_table( p_owner             => p_owner,
                            p_table             => p_table,
   ...
   BEGIN
      td_dbapi.build_indexes( p_owner             => p_owner,
                              p_table             => p_table,
