/* Formatted on 2007/05/12 02:59 (Formatter Plus v4.8.8) */
...
-- this is not an autonomous transaction because COREUTILS.exec_auto is
   ...
   BEGIN
      ...
      FOR c_constraints IN
         (SELECT
                 ...
                 || RANK () OVER (PARTITION BY con_e_ext ORDER BY constraint_name)
                                                                 con_e_rename,
                 constraint_ddl, owner, constraint_name, con_rename,
            ...
            FROM (SELECT REGEXP_REPLACE
                            (REGEXP_REPLACE
                             ...
                             CASE
                                -- if tablespace is provided, tack it on the end
                             WHEN coreutils.get_yn_ind (p_seg_attributes) =
                                                                         'yes'
                             AND p_tablespace IS NOT NULL
      ...
      LOOP
         ...
         o_app.log_msg ('Renamed constraint: ' || c_constraints.con_rename,
                        4);
         o_app.log_msg (   'Exception renamed constraint: '
         ...
         BEGIN
            coreutils.exec_auto (l_ddl, p_runmode => o_app.runmode);
            o_app.log_msg ('Constraint ' || c_constraints.con_rename
                           || ' built'
                          );
         ...
         EXCEPTION
            ...
            THEN
               o_app.log_msg
                        (   'Primary key constraint already exists on table '
                         || l_tab_name
                        );
            WHEN e_dup_not_null
            THEN
   ...
   IS
      ...
      l_idx_cnt    NUMBER        := 0;
      o_app        applog
               := applog (p_module       => 'drop_indexes',
                          p_runmode      => p_runmode);
   BEGIN
      o_app.log_msg ('Dropping indexes on ' || l_tab_name);

      FOR c_indexes IN (SELECT    'drop index '
                               || owner
                               || '.'
   ...
   BEGIN
      ...
      o_app.log_msg (   'Inserting records from '
                     || l_src_name
                     || ' into '
                     || l_trg_name,
                     3
   ...
   IS
      ...
      l_src_name        VARCHAR2 (61)
                                  := p_source_owner || '.' || p_source_object;
      l_trg_name        VARCHAR2 (61)    := p_owner || '.' || p_table;
      ...
      EXCEPTION
         ...
         THEN
            raise_application_error
                                  (coreutils.get_err_cd ('on_clause_missing'),
   ...
   IS
      l_rows   BOOLEAN := FALSE;
      o_app    applog
                := applog (p_module       => 'load_tables',
                           p_runmode      => p_runmode);
   BEGIN
      o_app.log_msg (   'Loading matching objects from the '
                     || p_source_owner
                     || ' schema.'
                    );
      ...
      FOR c_objects IN
         (SELECT o.owner src_owner, object_name src, t.owner targ_owner,
                 table_name targ
      ...
      THEN
         raise_application_error
                               (coreutils.get_err_cd ('incorrect_parameters'),
   ...
   IS
      l_src_name       VARCHAR2 (61)
                                   := p_source_owner || '.' || p_source_table;
      l_tab_name       VARCHAR2 (61)             := p_owner || '.' || p_table;
      l_target_owner   all_tab_partitions.table_name%TYPE
                                                       DEFAULT p_source_owner;
      l_rows           BOOLEAN                                  := FALSE;
            ...
            THEN
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     o_app.log_msg (   'No stats existed for partition '
                                    || p_partname,
                                    3
                                   );
               END;
      ...
      FOR c_idx IN
         (SELECT DISTINCT    'alter index '
                          ...
                          SUM (CASE idx_ddl_type
                                  ...
                                  ELSE 0
                               END
                              ) OVER (PARTITION BY 1) num_indexes,
                          SUM (CASE idx_ddl_type
                                  WHEN 'P'
                                     THEN 1
                                  ELSE 0
   ...
   PROCEDURE load_scd (
      ...
      p_owner           VARCHAR2,
      p_table           VARCHAR2,
      p_nat_key_col     VARCHAR2,
      p_type2_cols      VARCHAR2,
      p_sk_col          VARCHAR2 DEFAULT NULL,
      p_sk_seq_owner    VARCHAR2 DEFAULT NULL,
      p_sk_seq_name     VARCHAR2 DEFAULT NULL,
   ...
   IS
      l_src_name   VARCHAR2 (61) := p_source_owner || '.' || p_source_object;
      l_trg_name   VARCHAR2 (61) := p_owner || '.' || p_table;
      l_seq_name   VARCHAR2 (61) := p_sk_seq_owner || '.' || p_sk_seq_name;
      o_app        applog
      ...
      WITH type2_columns AS
           ...
           (SELECT     TRIM (SUBSTR (COLUMNS,
                  ...
                  FROM (SELECT ',' || p_type2_columns || ',' COLUMNS
                          FROM DUAL)
            CONNECT BY LEVEL <=
                            LENGTH (p_type2_columns)
                          - LENGTH (REPLACE (p_type2_columns, ',', ''))
                          + 1)
      SELECT    'SELECT CASE '
             || p_surr_key_col
             || ' WHEN -1 THEN '
        ...
        FROM (SELECT    'CASE '
                     ...
                     (SELECT REGEXP_REPLACE
                       ...
                       WHERE table_name = p_table
                         AND owner = p_owner
                         AND column_name NOT IN (SELECT COLUMNS
                                                   FROM DATA)) AS type1_rules,
                     (SELECT REGEXP_REPLACE (stragg (COLUMNS),
                                             ',',
                                             ',' || CHR (10),
                                             'i'
                                            )
