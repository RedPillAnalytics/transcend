CREATE OR REPLACE PACKAGE BODY common.file_mover
IS
...
-- SOURCE_DIR is configured in the FILE_CTL table
   ...
   AS
      l_max_bytes            file_ctl.max_bytes%TYPE;
      l_min_bytes            file_ctl.min_bytes%TYPE;
      l_jobname              file_ctl.jobname%TYPE;
      l_multi_files_action   file_ctl.multi_files_action%TYPE;
      l_app                  app_info
                             := app_info( p_module =>      'FILE_MOVER.AUDIT_FILE',
                                          p_debug =>       p_debug );
   BEGIN
      SELECT max_bytes,
             min_bytes,
      ...
      INSERT INTO file_dtl
           ...
           VALUES ( p_filename,
                    ...
                    p_jobnumber,
                    'N',
                    DECODE( REGEXP_SUBSTR( l_multi_files_action,
                                           '^All',
                                           1,
                                           1,
                                           'i' ),
   ...
   IS
      ...
      l_jobnumber    file_ctl.jobnumber%TYPE;
      l_source_dir   file_ctl.source_dir%TYPE;
      l_wrk_dir      file_ctl.wrk_dir%TYPE;
      l_arch_dir     file_ctl.arch_dir%TYPE;
   ...
   BEGIN
      CASE
         WHEN p_jobnumber IS NOT NULL
         THEN
            l_jobnumber := p_jobnumber;
         WHEN     ( p_jobnumber IS NULL )
              AND (     p_jobname IS NOT NULL
                    AND p_source_regexp IS NOT NULL )
         THEN
            BEGIN
               SELECT jobnumber
                 INTO l_jobnumber
                ...
                WHERE jobname = p_jobname
                  AND source_regexp = p_source_regexp;
            EXCEPTION
               WHEN NO_DATA_FOUND
               THEN
                  RAISE e_dup_file;
         ...
         ELSE
            raise_application_error
               ( -20001,
                 'Either P_JOBNUMBER or a combination of P_JOBNAME and P_SOURCE_REGEXP is required' );
      END CASE;
      ...
      SELECT source_dir,
        ...
        INTO l_source_dir,
             ...
             l_ext_dir
        FROM file_ctl
       WHERE jobnumber = l_jobnumber;

      IF    NVL( p_wrk_dir, l_wrk_dir ) = NVL( p_source_dir, l_source_dir )
         OR NVL( p_arch_dir, l_arch_dir ) = NVL( p_source_dir, l_source_dir )
         OR NVL( p_ext_dir, l_ext_dir ) = NVL( p_source_dir, l_source_dir )
      THEN
         raise_application_error( -20001,
                                  'Target directories cannot be the same as source directories' );
      END IF;

      l_app.set_action( 'Update FILE_CTL' );

      UPDATE file_ctl
         SET jobname = DECODE( p_jobname,
                               NULL, jobname,
                               p_jobname ),
             filename = DECODE( p_filename,
                                NULL, filename,
                                p_filename ),
             source_regexp = DECODE( p_source_regexp,
                                     NULL, source_regexp,
                                     p_source_regexp ),
             regexp_ci_ind = DECODE( p_regexp_ci_ind,
                                     NULL, regexp_ci_ind,
                                     p_regexp_ci_ind ),
             source_dir = DECODE( p_source_dir,
                                  NULL, source_dir,
                                  p_source_dir ),
             min_bytes = DECODE( p_min_bytes,
                                 NULL, min_bytes,
                                 p_min_bytes ),
             max_bytes = DECODE( p_max_bytes,
                                 NULL, max_bytes,
                                 p_max_bytes ),
             arch_dir = DECODE( p_arch_dir,
                                NULL, arch_dir,
                                p_arch_dir ),
             add_arch_ts_ind = DECODE( p_add_arch_ts,
                                       NULL, add_arch_ts_ind,
                                       p_add_arch_ts ),
             wrk_dir = DECODE( p_wrk_dir,
                               NULL, wrk_dir,
                               p_wrk_dir ),
             ext_dir = DECODE( p_ext_dir,
                               NULL, ext_dir,
                               p_ext_dir ),
             ext_filename = DECODE( p_ext_filename,
                                    NULL, ext_filename,
                                    p_ext_filename ),
             ext_table = DECODE( p_ext_table,
                                 NULL, ext_table,
                                 p_ext_table ),
             ext_tab_owner = DECODE( p_ext_tab_owner,
                                     NULL, ext_tab_owner,
                                     p_ext_tab_owner ),
             multi_files_action = DECODE( p_mul_fil_act,
                                          NULL, multi_files_action,
                                          p_mul_fil_act ),
             files_required_ind =
                                DECODE( p_files_req_ind,
                                        NULL, files_required_ind,
                                        p_files_req_ind ),
   ...
   EXCEPTION
      ...
      WHEN e_dup_file
      THEN
         l_app.set_action( 'Insert FILE_CTL' );
   ...
   PROCEDURE process_ext_table(
      p_jobnumber   VARCHAR2,
      p_debug       BOOLEAN DEFAULT FALSE )
   IS
                  ...
                  AND file_dt IN(
                         SELECT MIN( file_dt )
                          ...
                          WHERE jobnumber = p_jobnumber
                            AND session_id = SYS_CONTEXT( 'USERENV', 'SESSIONID' )
                            AND ext_tab_ind = 'N' );
            WHEN REGEXP_LIKE( c_file_ctl.multi_files_action,
                              '^(all).+',
                              'i' )
            THEN
   ...
   PROCEDURE audit_ext_table(
      p_jobnumber   VARCHAR2,
      p_debug       BOOLEAN DEFAULT FALSE )
   IS
      l_jobname              file_ctl.jobname%TYPE;
      l_ext_table            file_ctl.ext_table%TYPE;
      l_ext_tab_owner        file_ctl.ext_tab_owner%TYPE;
      l_multi_files_action   file_ctl.multi_files_action%TYPE;
      l_num_rows             NUMBER;
      l_num_lines            NUMBER;
      l_pct_miss             NUMBER;
   ...
   BEGIN
      ...
      IF l_multi_files_action = 'ALL AUTO'
      THEN
         alter_ext_table( l_ext_tab_owner,
                          l_ext_table,
                          TRUE,
                          p_debug );
      END IF;
      ...
      l_sql := 'SELECT count(*) FROM ' || l_ext_tab_owner || '.' || l_ext_table;
   ...
   PROCEDURE error_ext_table(
      p_debug   BOOLEAN DEFAULT FALSE )
   IS
      l_app   app_info := app_info( p_module =>      'FILE_MOVER.ERROR_EXT_TABLE',
                                    p_debug =>       p_debug );
   BEGIN
      UPDATE file_dtl
         SET ext_tab_ind = 'E'
   ...
   BEGIN
      ...
      FOR c_file_ctl IN ( SELECT  REGEXP_REPLACE( source_dir,
                                                  '//',
                                                  '/',
                                                  1,
                                                  1,
                                                  'i' ) source_dir,
                                  REGEXP_REPLACE( arch_dir,
                                                  '//',
                                                  '/',
                                                  1,
                                                  1,
      ...
      LOOP
         l_rows_dtl := FALSE;
         l_rows_list := FALSE;
         job.log_msg(    'Jobnumber: '
                      || c_file_ctl.jobnumber
                      || ', Regexp: '
         ...
         LOOP
            ...
            IF     c_file_ctl.wrk_dir IS NOT NULL
               AND c_dir_list.archfile <> c_dir_list.wrkfile
            THEN
         ...
         CASE
            WHEN NOT l_rows_list
            AND c_file_ctl.files_required_ind = 'Y'
            THEN
               raise_application_error( -20001, 'No files found... FILES_REQUIRED_IND is "Y"' );
            WHEN NOT l_rows_list
            AND c_file_ctl.files_required_ind = 'N'
            THEN
            ...
            THEN
               ...
               LOOP
                  ...
                  IF     c_file_ctl.ext_filename IS NOT NULL
                     AND c_file_dtl.ext_filename IS NOT NULL
                  THEN
                     ...
                     ELSE
                        util.run_cmd( l_cmd );
                        job.log_msg( c_file_dtl.archfilename || ' copied to '
                                     || c_file_dtl.ext_file );
                     END IF;
                  END IF;
                  ...
                  l_filename :=
                              utility.unzip_file( c_file_ctl.ext_dir,
                                                  c_file_dtl.filename,
                                                  p_debug );
