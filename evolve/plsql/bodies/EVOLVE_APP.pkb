...
AS
   ...
   AS
      ...
      l_client_id         all_scheduler_jobs.client_id%TYPE
                                                        := l_module || '-' || l_action || '-' || l_session_id;
   ...
   BEGIN
      ...
      LOOP
         ...
         SELECT COUNT( * )
          ...
          WHERE client_id = l_client_id AND state = 'RUNNING';
   ...
   -- uses DBMS_SCHEDULER to submit mail
   ...
   AS
      ...
      PRAGMA EXCEPTION_INIT( e_invalid_jobname, -23481 );
   BEGIN
      -- variable to hold the mail call
      l_mail_sql :=
         ...
         || p_mime_type
         || '''); end;';
      evolve_log.log_msg( 'The UTL_MAIL call is: ' || l_mail_sql, 5 );
